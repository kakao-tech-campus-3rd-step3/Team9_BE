...
@ApiResponses({
        @ApiResponse(responseCode = "403", description = "접근 권한 없음 (스터디 리더가 아닌 경우)", content = @Content(
                schema = @Schema(implementation = ErrorResponseDto.class),
                examples = @ExampleObject(
                        name = "권한 없음 예시",
                        value = "{\"error_code\": \"FORBIDDEN_STUDY_LEADER_ONLY\", \"message\": \"스터디 리더만 접근할 수 있습니다.\"}"
                )
        ))
})
