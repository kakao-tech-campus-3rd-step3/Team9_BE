...
public class StudyRankingServiceImpl implements StudyRankingService{
    ...
    @Override
    public MyRankResponseDto getMyRank(Long studyId, User user) {
        validateStudyExists(studyId);
        List<RankerResponseDto> totalRanking = getRankedStudyMembers(studyId);

        return totalRanking.stream()
                .filter(ranker -> ranker.userId().equals(user.getId()))
                .findFirst()
                .map(ranker -> new MyRankResponseDto(ranker.rank(), ranker.score()))
                .orElseThrow(() -> new BusinessException(ErrorCode.FORBIDDEN_STUDY_MEMBER_ONLY, "사용자가 해당 스터디의 멤버가 아닙니다."));
    }

    @Override
    public TotalRankingResponseDto getTotalRanking(Long studyId, User user) {
        validateStudyExists(studyId);
        validateStudyMember(studyId, user.getId());
