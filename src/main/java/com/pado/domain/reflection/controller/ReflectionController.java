...
@RestController
@RequestMapping("/api/studies/{study_id}/reflections")
...
public class ReflectionController {
    ...
    @Api403ForbiddenStudyMemberOnlyError
    @GetMapping
    public ResponseEntity<ReflectionListResponseDto> getReflections(
        @Parameter(description = "회고를 조회할 스터디 ID") @PathVariable("study_id") Long studyId,
        @Parameter(hidden = true) @CurrentUser User user,
        @Parameter(description = "내 회고만 조회 여부", example = "me") @RequestParam(name = "author", required = false) String author,
        @ParameterObject @PageableDefault(size = 10) Pageable pageable
    ) {
        return ResponseEntity.ok(reflectionService.getReflections(studyId, user, author, pageable));
    }
    ...
    @Api403ForbiddenStudyMemberOnlyError
    @GetMapping("/{reflection_id}")
    public ResponseEntity<ReflectionResponseDto> getReflection(
        @Parameter(description = "조회할 스터디 ID") @PathVariable("study_id") Long studyId,
        @Parameter(description = "조회할 회고 ID") @PathVariable("reflection_id") Long reflectionId,
        @Parameter(hidden = true) @CurrentUser User user) {
        return ResponseEntity.ok(reflectionService.getReflection(studyId, reflectionId, user));
    }
    ...
    @PatchMapping("/{reflection_id}")
    public ResponseEntity<ReflectionResponseDto> updateReflection(
        @Parameter(description = "수정할 회고가 속한 스터디 ID") @PathVariable("study_id") Long studyId,
        @Parameter(description = "수정할 회고 ID") @PathVariable("reflection_id") Long reflectionId,
        @Parameter(hidden = true) @CurrentUser User user,
        @Valid @RequestBody ReflectionCreateRequestDto request
    ) {
        return ResponseEntity.ok(
            reflectionService.updateReflection(studyId, reflectionId, user, request));
    }
    ...
    @Api404ReflectionNotFoundError
    @Api403ForbiddenReflectionOwnerError
    @DeleteMapping("/{reflection_id}")
    public ResponseEntity<Void> deleteReflection(
        @Parameter(description = "삭제할 회고가 속한 스터디 ID") @PathVariable("study_id") Long studyId,
        @Parameter(description = "삭제할 회고 ID") @PathVariable("reflection_id") Long reflectionId,
        @Parameter(hidden = true) @CurrentUser User user) {
        reflectionService.deleteReflection(studyId, reflectionId, user);
